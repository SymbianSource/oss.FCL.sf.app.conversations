...
TInt CVIMPSTSettingsStoreImp::GetL(TUint32 aServiceId, TSettingItemName aSettingItemNAme, RBuf& aBuffer )
    {
    if(aSettingItemNAme == EServiceToneFileName)
        {
        _LIT(filename,"testtonefile");
        aBuffer.Copy(filename);
        }
